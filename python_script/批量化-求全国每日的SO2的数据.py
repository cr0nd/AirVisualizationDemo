...
def get_wind_data_by_day(csv_path,save_path):
    ...
    with open(csv_path, 'r', encoding='utf-8') as f:
        ...
        for index, row in enumerate(reader):
            ...
            # lat 纬度 lon 经度
            ans.append([float(row[12]),float(row[11]),float(row[2])])
...
def get_wind_infomation(a1,a2,a3,b1,b2,b3):
    ...
    for i in range((end-begin).days+1):
        ...
        save_dir = "./data/so2_data/{}".format(day.strftime("%Y%m"))
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        save_path = "./data/so2_data/{}/so2_{}.json".format(day.strftime("%Y%m"),day.strftime("%Y%m%d"))
