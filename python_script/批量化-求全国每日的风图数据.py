...
def get_wind_data_by_day(csv_path,save_path):
    ...
    with open(csv_path, 'r', encoding='utf-8') as f:
        ...
        for index, row in enumerate(reader):
            ...
            v = float(row[7])
            mag = (v*v+u*u)**0.5
            ans.append([float(row[12]),float(row[11]),float(row[6]),float(row[7]),mag])
...
def get_wind_infomation(a1,a2,a3,b1,b2,b3):
    ...
    for i in range((end-begin).days+1):
        ...
        save_dir = "./data/wind_data/{}".format(day.strftime("%Y%m"))
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        save_path = "./data/wind_data/{}/wind_{}.json".format(day.strftime("%Y%m"),day.strftime("%Y%m%d"))
